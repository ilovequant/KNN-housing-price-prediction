...
#data = pd.read_csv("/Users/Alex/Downloads//data.csv")
#dataArray = np.array(df)      #convert dataframe into np array


def gaussian(dist, sigma):          #Gaussian kernel
    #tempg= np.array(np.power(-dist,2)/(2*sigma**2),dtype=np.float32)
    dist = np.array(dist, dtype = np.float32) 
    tempg= np.power((2*np.pi), -0.5)*np.exp(-np.power(dist, 2)/2)
    #inv = np.array(1/np.abs(dist))  #inverse kernel
    return tempg
...
##measure performance
##RMSE median of absolute value of (prediction - actual value) over actual value
...
def main():
    ...
    performanceEval(test[:,3], np.array(testSetPred))   #performance metrics
    print(test[0:2, 3])
